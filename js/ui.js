...
'rejectionCodesBtn', 'interfacesHomeBtn', 'rejectionCodesPanel', 'closeRejectionCodes',
...
export function renderInterfaceDetails(id) {
    ...
    const eventSummary = Array.isArray(item.eventCodes) && item.eventCodes.length
        ? item.eventCodes.map(code => `<code class="event-code">${escapeHtml(code)}</code>`).join('')
        : '<span class="summary-muted">Not catalogued</span>';
    const hasReferenceNotes = !!(item.verification || item.releaseStatus || item.notes?.length || item.routingRules?.length || item.errorCodes?.length);

    DOMElements.detailsContent.innerHTML = `
        <nav class="page-breadcrumb" aria-label="Breadcrumb">
            <span>Interfaces</span><span aria-hidden="true">/</span><strong>${escapeHtml(item.id)}</strong>
        </nav>
        ...
        <section class="summary-strip" aria-label="Interface summary">
            ...
            <div class="summary-cell summary-events">
                <span class="summary-label">Events</span>
                <div class="summary-value event-code-list">${eventSummary}</div>
            </div>
            <div class="summary-cell">
                <span class="summary-label">Supplier role</span>
                <div class="summary-value">${escapeHtml(supplier.label)}</div>
            </div>
        </section>

        <nav class="detail-section-nav" aria-label="On this interface">
            <a href="#overview">Overview</a>
            <a href="#message-structure">Message structure</a>
            ${hasReferenceNotes ? '<a href="#reference-notes">Reference notes</a>' : ''}
        </nav>

        <section id="overview" class="content-card context-card page-section">
        ...
        ${hasReferenceNotes ? `<section id="reference-notes" class="reference-notes-section page-section">
            <div class="page-section-heading">
                <p class="section-kicker">Reference notes</p>
                <h3>Status, routing &amp; supporting notes</h3>
            </div>
...
function generateDipStructureHtml(interfaceItem) {
    ...
    return `<section id="message-structure" class="dip-structure-card page-section mt-6 md:mt-8">
...
export function showRejectionCodes() {
    ...
    DOMElements.rejectionCodesBtn?.setAttribute('aria-current', 'page');
    DOMElements.interfacesHomeBtn?.classList.remove('is-active');
    DOMElements.interfacesHomeBtn?.removeAttribute('aria-current');
    DOMElements.welcomeMessage.classList.add('hidden');
...
export function hideRejectionCodes() {
    ...
    DOMElements.rejectionCodesBtn?.removeAttribute('aria-current');
    DOMElements.interfacesHomeBtn?.classList.add('is-active');
    DOMElements.interfacesHomeBtn?.setAttribute('aria-current', 'page');
...
export function renderRejectionCodesList(codes) {
    ...
    DOMElements.rejectionCodesList.innerHTML = codes.map(([codeId, code]) => `
        <article class="rejection-code-card">
            <div class="rejection-code-id"><span class="code-pill">${escapeHtml(codeId)}</span></div>
            <div class="rejection-code-body">
                <h3>${escapeHtml(code.description)}</h3>
                ${code.reason ? `<p><strong>Reason:</strong> ${escapeHtml(code.reason)}</p>` : ''}
                ${code.resolution ? `<p><strong>Resolution:</strong> ${escapeHtml(code.resolution)}</p>` : ''}
                ${code.repeatForQueue === true ? `<p class="rejection-meta">Repeat for Queue: Yes</p>` : ''}
            </div>
        </article>`).join('');
}
