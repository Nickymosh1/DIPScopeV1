...
import { findTechnicalNode, schemaFacts } from './technicalSchema.js';
...
export function bindReferencePanel() {
  ...
  document.addEventListener('click', event => {
    ...
    const technicalNode = findTechnicalNode(iface, variant, blockButton?.dataset.nodeKey);
    const technicalField = technicalNode?.fields.find(f => f.itemId === id && (!link.dataset.fieldKey || f.key === link.dataset.fieldKey));
    const key = technicalField?.key || block?.payloadKeys?.[id];
    ...
    const extra = schemaFacts(technicalField?.schema || item.technicalDefinition?.schema, technicalField?.required) + Object.entries(item).filter(([name]) => !['technicalContexts','technicalDefinition','name','cmo','rule','populationNotes','example','payloadKey'].includes(name)).map(([name,value]) => section(name.replace(/([A-Z])/g, ' $1').replace(/^./, c => c.toUpperCase()), typeof value === 'object' ? JSON.stringify(value,null,2) : value)).join('');
